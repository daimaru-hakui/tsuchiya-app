...
export async function updateSku(data: UpdateSku, productId: string, skuId: string): Promise<{ message: string | undefined; }> {
  ...
  try {
    const skuRef = db.collection("products").doc(productId).collection("skus").doc(skuId);
    await skuRef.update({
